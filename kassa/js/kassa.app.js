...
var kassa = {
    ...
    run: (function() {
        ...
        var currentWindow = chrome.app.window.current().innerBounds;
        chrome.app.window.onBoundsChanged.addListener(resizeHandler);

        setInterval(resizeHandler, 1000);
        $('#products').on('scroll', scrollHandler);
        $('#products').css('height', (currentWindow.height-50));
        $('.order-list').css('height', (currentWindow.height-200));
        $('.scroll').css('height', (currentWindow.height-50));

        $('#scroll-products .scroll-bar').draggable({
            axis: "y",
            containment: [15, 130, 15, (currentWindow.height-160)]
        });
        ...
        $('#categories').on('click', 'h3', function() {
            ...
            updateScroll();
            getURL(product.image, product.id);
            updateImages();
        });
            ...
            if(product.categories_id_sub == null) {
                ...
                subcategory.products.forEach(function(subproduct) {
                    if(subproduct.visible) {
                        $('#row-subcategory-'+subcategory.id).append('<div class="col-lg-2 col-md-3 col-sm-3"><div class="thumbnail product" data-product-id="'+subproduct.id+'"><img src="" alt="'+subproduct.name+'" class="img-product-'+subproduct.id+'"><div class="caption"><p class="product-info"><span class="product-name">'+subproduct.name+'</span><span class="product-price-plus">'+subproduct.price+'</span></p></div></div></div>');
                        getURL(subproduct.image, subproduct.id);
                    }
                });
            }
        });
        ...
        $.post(kassa.api+'products/get', {uid: kassa.uid, secret: kassa.secret}).done(function(data) {
            ...
            categories.forEach(function(category) {
                ...
                category.products.forEach(function(product) {
                    $('#row-category-'+category.id).append('<div class="col-lg-2 col-md-3 col-sm-3" id="products-'+product.id+'"><div class="thumbnail product" data-product-id="'+product.id+'"><img src="" alt="'+product.name+'" class="img-product-'+product.id+'"><div class="caption"><p class="product-info"><span class="product-name">'+product.name+'</span><span class="product-price">'+product.price+'</span></p></div></div></div>');
                    getURL(product.image, product.id);
                    if(product.visible) {
        ...
        function getURL(url, productId) {
            var xhr = new XMLHttpRequest();
            xhr.open('GET', url, true);
            xhr.responseType = 'blob';
            xhr.onload = function(e) {
                $('.img-product-'+productId).attr('src', window.URL.createObjectURL(this.response));
            };
            xhr.send();
        }
    ...
    setup: (function() {
        $(document).ready(function() {
            var storage = chrome.storage.local.get(['kiosk'], function(vals) {
                var storage = vals['kiosk'];
                if(!storage) {
                    $('#main').addClass('blurable');
                    $('#main-overlay').show();
                } else {
                    var setup = JSON.parse(storage);
                    kassa.api = setup.api;
                    kassa.uid = setup.uid;
                ...
                $('.setup-submit').click(function() {
                    ...
                    $.post(kassa.api+'check', {uid: kassa.uid, secret: kassa.secret}, function(data) {
                        if(data.success) {
                            chrome.storage.local.set({"kiosk": JSON.stringify({
                                api: kassa.api,
                                uid: kassa.uid,
                                secret: kassa.secret
                            })});
